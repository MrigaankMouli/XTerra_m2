...
def energy(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    ...
    return torch.sum(torch.abs(qvel) * torch.abs(qfrc) + torch.pow(torch.abs(qfrc), 0.0002), dim=-1) # penalizing high torques more to encourage smoother motions --> given in the M2_legged_rl repository
