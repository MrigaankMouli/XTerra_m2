\section{Adding the M2 Flat Locomotion Task to \texttt{unitree\_rl\_lab}}

The M2 flat locomotion task was added as a new Isaac Lab extension package inside the existing
\texttt{unitree\_rl\_lab} style repository. Instead of modifying the Unitree Go2 task in-place, I created
a separate package named \texttt{M2\_rl} under \texttt{source/M2\_rl}. This kept the M2 robot
...
The M2 task is exposed to Isaac Lab through Gymnasium registration. The package initializer
\texttt{M2\_rl/tasks/\_\_init\_\_.py} imports all task subpackages using Isaac Lab's package import
...
\texttt{M2\_rl/tasks/locomotion/robots/M2/\_\_init\_\_.py} with the environment id
\texttt{M2-Velocity}. This registration maps the id to Isaac Lab's
...
\texttt{M2\_rl.tasks.locomotion.agents.rsl\_rl\_ppo\_cfg}.

This design makes the task discoverable by the existing command-line workflow. After installing the
extension with \texttt{./M2\_rl.sh --install}, the environment can be trained and played using the same
script structure as the Unitree tasks:

\begin{verbatim}
./m2_rl.sh --train --task M2-Velocity
./m2_rl.sh --play  --task M2-Velocity
\end{verbatim}

\subsection{M2 Robot Asset Configuration}

The robot description is defined in \texttt{M2\_rl/assets/robots/M2.py}. I added a
\texttt{M2ArticulationCfg} and used a USD-based spawn configuration for the M2 model. The
configuration points to the inspected \texttt{M2\_imu.usd} asset, enables contact sensors, and sets the
rigid body and articulation properties required by Isaac Lab. The rigid body parameters were adapted from
the earlier M2 legged RL setup, while the articulation solver settings follow the Unitree Go2-style
configuration.

The M2-specific initial state and actuator model were also defined in this file. The default base
...
\texttt{M2\_rl/tasks/locomotion/robots/M2/velocity\_env\_cfg.py}. The scene configuration replaces
the Unitree robot with \texttt{M2\_CFG} and places it at the prim path
\texttt{\{ENV\_REGEX\_NS\}/M2}. I updated all M2-specific prim and body names after inspecting the
USD stage in Isaac Sim. For example, the base body is referenced as \texttt{base\_link}, the height scanner
uses \texttt{\{ENV\_REGEX\_NS\}/M2/M2/base\_link}, and contact sensors are attached under
\texttt{\{ENV\_REGEX\_NS\}/M2/M2/.*}. These changes were important because the original Go2 task
...
The action space is a joint position action over all M2 joints. The action scale is \texttt{0.25}, and
...
the weights and additional penalties were adjusted for M2. Positive reward terms track commanded
...
\texttt{M2\_rl/tasks/locomotion/mdp/rewards.py}, including energy usage, roll penalty, pitch-rate
...
uses \texttt{base\_link}, matching the M2 USD body name, and the bad-orientation termination uses a
...
\texttt{M2\_rl/tasks/locomotion/agents/rsl\_rl\_ppo\_cfg.py}. I reused the RSL-RL on-policy runner
...
checkpoints and exported policies are stored under \texttt{logs/rsl\_rl/M2\_velocity}, confirming that
the flat M2 task was integrated into the same training and export workflow as the original
\texttt{unitree\_rl\_lab} tasks.

\begin{table}[h]
    \centering
    \caption{Main files added or modified for the M2 flat locomotion task.}
    \begin{tabular}{p{0.38\linewidth} p{0.52\linewidth}}
        \hline
        \textbf{File} & \textbf{Purpose} \\
        \hline
        \texttt{source/M2\_rl} & New Isaac Lab extension package for M2 tasks. \\
        \texttt{assets/robots/M2.py} & Defines the M2 USD articulation, initial pose, actuators, and joint ordering. \\
        \texttt{tasks/locomotion/robots/M2/\_\_init\_\_.py} & Registers the flat task as \texttt{M2-Velocity}. \\
        \texttt{tasks/locomotion/robots/M2/velocity\_env\_cfg.py} & Defines the flat scene, observations, actions, commands, rewards, events, curriculum, and terminations. \\
        \texttt{tasks/locomotion/mdp/rewards.py} & Adds M2-specific reward and penalty terms. \\
        ...
        \texttt{M2\_rl.sh} & Provides install, list, train, and play entry points for the M2 extension. \\
        \hline
    \end{tabular}
    \label{tab:M2_flat_task_files}
\end{table}

Overall, the M2 flat locomotion task was integrated by following the same modular structure as the
...
work was replacing Go2-specific robot assumptions with M2 USD prim paths, body names, joint names,
actuator parameters, and reward shaping suitable for the M2 morphology.
