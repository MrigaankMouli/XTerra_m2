\section{Extending the M2 Locomotion Task to Rough Terrain}

After the flat M2 velocity-tracking task was implemented, the existing task was extended into a rough
...
robot task folder,
\texttt{M2\_rl/tasks/locomotion/robots/M2-rough}, while reusing the same M2 articulation,
...
\texttt{M2-Velocity-Rough}. This allowed the rough environment to appear as a separate task in the
...
gym.register(
    ...
    kwargs={
        ...
        "rsl_rl_cfg_entry_point": (
            "m2_rl.tasks.locomotion.agents.rsl_rl_ppo_cfg:"
            "BasePPORunnerCfg"
        ),
    },
)
...
./m2_rl.sh --train --task M2-Velocity-Rough
\end{lstlisting}
...
roughness, pyramid slopes, ascending stairs, and descending stairs. This gave the M2 policy exposure to
different contact patterns and body-pose disturbances during training.

\begin{lstlisting}[language=Python,caption={Mixed rough-terrain generator used by the M2 rough task.}]
...
The same M2 robot asset from the flat task was then spawned on top of this generated terrain.
...
robot: ArticulationCfg = ROBOT_CFG.replace(
    prim_path="{ENV_REGEX_NS}/M2"
)
\end{lstlisting}

The height scanner and contact sensors were kept from the M2 task and pointed to the M2 USD prim
paths. The scanner was mounted relative to \texttt{base\_link}, while the contact sensor matched all M2
...
Overall, the rough-terrain extension was implemented by taking the already working M2 flat locomotion
...
modified to train the M2 policy on random roughness, slopes, ascending stairs, and descending stairs.
