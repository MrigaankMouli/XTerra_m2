...
This document covers the Isaac Lab task definitions for M2 flat and rough terrain locomotion.

---

## 1. Extension and Task Registration

Tasks are exposed to Isaac Lab via Gymnasium registration. The flat task is registered with id `M2-Velocity`, the rough task with `M2-Velocity-Rough`, and the stair traversal task with `M2-Velocity-Stairs`.

### Task Registration

```python
# source/m2_rl/m2_rl/tasks/locomotion/robots/[TASK_NAME]/__init__.py
gym.register(
    id="[TASK-NAME]",
...
The scene replaces the Unitree Go2 asset with `M2_CFG` and updates all prim/body name references after inspecting the USD stage in Isaac Sim.

| Property | Value |
|---|---|
| Robot prim path | `ENV_REGEX_NS/M2` |
| Base body name | `base_link` |
| Contact sensor path | `ENV_REGEX_NS/M2/M2/.*` |
...
Joint position targets for all 12 M2 joints. Actions are scaled by **0.25** and applied as offsets from the nominal standing posture.
