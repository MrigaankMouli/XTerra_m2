...
This document covers how to define and validate the M2 robot as an Isaac Lab articulation.

---

## 1. ArticulationCfg

Isaac Lab uses `ArticulationCfg` to describe the robot's physical structure, initial state, and actuator models. The M2 configuration is defined in `source/m2_rl/m2_rl/assets/robots/M2.py`.

### USD Reference

```python
M2_CFG = ArticulationCfg(
    prim_path="/World/Origin0/Robot",
    spawn=sim_utils.UsdFileCfg(
        usd_path="/path/to/M2_imu.usd",
...
Values for damping, max velocities, and solver iterations are adopted from the official M2 Legged RL repository by Addverb.
...
## 5. Joint Naming: M2 vs. Unitree Go2

The M2 model uses a different joint naming scheme from the Unitree Go2. The regular expressions in the articulation config must match M2's convention.
...
Isaac Lab uses `ArticulationCfg` to describe the robot's physical structure, initial state, and actuator models. The M2 configuration is defined in `assets/robots/M2.py`.
